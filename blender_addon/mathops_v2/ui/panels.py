...
class MATHOPS_V2_PT_render_settings(Panel):
    ...
    def draw(self, context):
        ...
        layout.separator()
        march_col = layout.column(align=True)
        march_col.prop(settings, "surface_epsilon")
        march_col.prop(settings, "max_steps")
        march_col.prop(settings, "max_distance")
        layout.separator()
        layout.prop(settings, "disable_surface_shading")
        layout.separator()
        layout.prop(settings, "show_meshes")
        if settings.show_meshes:
            mesh_col = layout.column()
            mesh_col.prop(settings, "mesh_backface_culling")
        layout.separator()
